...
| `POST /api/events/registrations/{id}/payment/cash` | `events:manage` | Record a cash payment directly — creates and verifies a `Payment` in one call, no proof file | `404` unknown registration; `400` amount not greater than zero; `409` registration already has a submitted or verified payment |
